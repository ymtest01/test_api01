...
class SendRequests():
    ...
    def sendRequests(self,s,apiData):
        ...
        if apiData["输入数据"] == "":
            ...
            if apiData["运行次数"] == 2:
                # for i in range(1, 3):
                    if "$event_timestamp" in body_data:
                        # e=body_data["$event_timestamp"]
                        e = round(time.time() * 1000)
                        body_data["$event_timestamp"] = e
                        print(e)
            ...
            if datatype == "data":
                ...
                body = json.dumps(body_data)
                if "$ip" in body:     #ip参数化，判断是否存在
                    t='10.11.12.%d'%random.randint(0,20)   #随机产生ip

                    body=body.replace("$ip",t)     #替换ip
                    print(body)
